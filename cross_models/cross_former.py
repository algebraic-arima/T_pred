...
from cross_models.fft import FFTModel
...
class Crossformer(nn.Module):
    def __init__(self, data_dim, in_len, out_len, seg_len, win_size = 4,
        ...
        self.b = nn.Parameter(torch.randn(1)* 0.4)
        ...
        # fft model
        self.fft_model = FFTModel(data_dim, in_len, out_len, device=device)
    ...
    def forward(self, x_seq):
        ...
        y_seq = self.fft_model(x_seq)

        x_seq = self.enc_value_embedding(x_seq)
        # [batch, dim, in_len // seg_len, d_model]
        x_seq += self.enc_pos_embedding
        x_seq = self.pre_norm(x_seq)
        
        enc_out = self.encoder(x_seq)
        # [e_blocks + 1, batch, dim, in_len // seg_len // some win_size, d_model]
        dec_in = repeat(self.dec_pos_embedding, 'b ts_d l d -> (repeat b) ts_d l d', repeat = batch_size)
        # [batch, dim, out_len // seg_len, d_model]
        predict_y = self.decoder(dec_in, enc_out)
        # [batch, out_len, dim]

        return base + predict_y[:, :self.out_len, :] - 0.2 * y_seq[:, :self.out_len, :]