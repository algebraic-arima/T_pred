...
class Dataset_MTS(Dataset):
    ...
    def __read_data__(self):
        ...
        df_data = df_raw[cols_data] # all time series data
    ...
    def __wash__(self):
        diff = np.diff(self.data_x, axis=0)
        is_stable = np.all(diff <= 1e-4, axis=1)
        non_stable_indices = np.where(~is_stable)[0] + 1
        non_stable_indices = np.insert(non_stable_indices, 0, 0)
        self.data_x = self.data_x[non_stable_indices]
        self.data_y = self.data_y[non_stable_indices]
