...
root_path = "./music/"
file_name = "BACH.csv"
timestamps = np.arange(len(time_series)).reshape(-1, 1)
time_series_with_timestamps = np.hstack((timestamps, time_series))

df = pd.DataFrame(time_series_with_timestamps, columns=['timestamp', 's', 'a', 't', 'b'])

df.to_csv(root_path + file_name, index=False)
